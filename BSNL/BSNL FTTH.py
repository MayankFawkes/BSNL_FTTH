...
class frameBOTTOM(tkinter.Frame):
    ...
    def fetch(self):
        r = self._http("post", "https://redirect2.bbportal.bsnl.co.in/portal/fetchUserQuotaPM.do")
        data = dict()
        print(r)
    ...
    def refresh(self):
        ...
        if avia:
            ...
            usedtillnow, typeU = findall('([0-9]+.?[0-9]*) ([A-Z]{2})', data["row"][0]["totalUsed"])[0]
            totalwehave, typeT = findall('([0-9]+.?[0-9]*) ([A-Z]{2})', data["row"][0]["total"])[0]
    ...
    def widget(self):
        ...
        labels =["Today", "Total Used", "Service Name", "Total GBs", "Speed", "FUP Speed"]
        ...
        tkinter.Button(self, text="REFRESH", height=2, width=10, command=lambda: Thread(target=self.refresh).start()).grid(row=5,column=0, padx=10,pady=20)

        tkinter.Button(self, text="INFO", height=2, width=10, command=INFObox).grid(row=5,column=1, padx=10,pady=20)

        tkinter.Button(self, text="UPDATE", height=2, width=10, command=lambda: Thread(target=self.SoftwareUpdate).start()).grid(row=5,column=2, padx=10,pady=20)

        tkinter.Label(self, textvariable=self.isupdateaviable).grid(row=5,column=3, padx=10,pady=20)
